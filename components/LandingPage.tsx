  ...
  const coverageCategories = [
    {
      ...
      items: ["General Liability (GL)", "Professional Liability (E&O)", "Excess & Umbrella", "Directors & Officers (D&O)", "Employment Practices (EPLI)"],
      icon: "🛡️"
    },
    {
      title: "Operations & Assets",
      items: ["Commercial Property", "Business Owner’s Policy (BOP)", "Inland Marine", "Equipment Breakdown", "Business Interruption"],
      icon: "🏗️"
    },
    {
      title: "Fleet & Logistics",
      items: ["Commercial Auto", "Hired & Non-Owned Auto", "Motor Truck Cargo", "Freight Forwarding Liability", "Fleet Telematics Support"],
      icon: "🚛"
    },
    {
      title: "Specialized Risk",
      items: ["Workers’ Compensation", "Cyber Liability & Breach", "Pollution & Environmental", "Product Recall", "Crime & Fidelity Bond"],
      icon: "⚡"
    }
  ];
      ...
      <section className="relative min-h-[80vh] flex items-center px-6 overflow-hidden">
        <div className="absolute inset-0 z-0 opacity-20 grayscale brightness-50">
          <img 
            ...
            alt="Commercial Insurance Boss Background"
          />
        ...
        <div className="relative z-20 max-w-7xl mx-auto w-full flex flex-col lg:flex-row items-center gap-16 py-20">
          <div className="flex-1 text-center lg:text-left">
            <div className="inline-block px-4 py-1 bg-[#EAB308] text-black text-[10px] font-black rounded-full mb-6 tracking-[0.2em] uppercase">
              EXECUTIVE DISTRIBUTION
            </div>
            <h1 className="text-5xl md:text-[90px] font-black leading-[0.85] mb-8 tracking-tighter text-white">
          ...
          <div className="w-full max-w-xl">
            {!submitted ? (
              ...
              <div className="bg-[#111] border border-white/10 rounded-[32px] p-12 text-center shadow-2xl animate-in zoom-in-95 duration-500">
                ...
                <Button variant="outline" className="w-full" onClick={() => setSubmitted(false)}>New Inquiry</Button>
              </div>
            )}
          </div>
        </div>
      ...
      <section className="py-24 px-6 bg-[#0d0d0d] border-y border-white/5">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16">
            <h2 className="text-4xl md:text-6xl font-black tracking-tighter mb-4 text-white">Coverage Solutions</h2>
            <p className="text-gray-500 font-bold tracking-widest text-xs uppercase">Market-Leading Products for Scale</p>
          </div>
  ...
  const renderStep = () => {
    switch(step) {
      case 1:
        return (
          <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
            <h2 className="text-3xl font-black tracking-tighter mb-8 text-white">Company Identity</h2>
            <div className="space-y-4">
              <FormInput label="Business Legal Name *" value={formData.businessName} onChange={v => setFormData({...formData, businessName: v})} placeholder="Legal Entity Name" />
              <FormInput label="DBA (Doing Business As)" value={formData.dba} onChange={v => setFormData({...formData, dba: v})} placeholder="Trade Name" />
              <div className="grid grid-cols-2 gap-4">
                <FormInput label="Federal EIN *" value={formData.fein} onChange={v => setFormData({...formData, fein: v})} placeholder="XX-XXXXXXX" />
                <FormInput label="Years Active *" value={formData.yearsInBusiness} onChange={v => setFormData({...formData, yearsInBusiness: v})} placeholder="Age of Business" />
              </div>
            </div>
          </div>
        );
      case 2:
        return (
          <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
            <h2 className="text-3xl font-black tracking-tighter mb-8 text-white">Headquarters</h2>
            <div className="space-y-4">
              <FormInput label="HQ Address *" value={formData.address} onChange={v => setFormData({...formData, address: v})} placeholder="..." />
      ...
      case 3:
        const industries = ["Logistics & Fleet", "Tech & Cyber", "Manufacturing", "Medical & Pharma", "Professional Services", "Real Estate Dev", "Hospitality", "Wholesale", "Agriculture", "Construction", "Auto Dealership", "Food & Beverage", "Retail", "Other"];
        return (
          <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
            <h2 className="text-3xl font-black tracking-tighter mb-8 text-white">Industry</h2>
      ...
      case 5:
        return (
          <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
            <h2 className="text-4xl font-black tracking-tighter mb-8 text-white">Policy Profile</h2>
            ...
            <ToggleQuestion label="Know Monthly Premium?" value={formData.knowsPremium} onChange={v => setFormData({...formData, knowsPremium: v})} />
            <ToggleQuestion label="Declarations Ready?" value={formData.hasDeclarations} onChange={v => setFormData({...formData, hasDeclarations: v})} />
          </div>
        );
      case 7:
        return (
          <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
            <h2 className="text-3xl font-black tracking-tighter mb-8 text-white">Authorized Contact</h2>
            <div className="space-y-4">
              <FormInput label="Decision Maker Name *" value={formData.contactName} onChange={v => setFormData({...formData, contactName: v})} placeholder="..." />
              <FormInput label="Work Email *" type="email" value={formData.email} onChange={v => setFormData({...formData, email: v})} placeholder="..." />
              {/* FIXED: Changed setAppData/appData to setFormData/formData */}
              <FormInput label="Direct Line *" type="tel" value={formData.phone} onChange={v => setFormData({...formData, phone: v})} placeholder="..." />
            </div>
          </div>
        );
      case 8:
        return (
          <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
            <h2 className="text-3xl font-black tracking-tighter mb-8 text-white">Final Authorization</h2>
            <div className="bg-white/5 border border-white/10 rounded-2xl p-6 space-y-4">
              <div className="flex justify-between border-b border-white/5 pb-2">
                <span className="text-gray-400 text-xs font-bold tracking-widest uppercase">Entity</span>
                <span className="font-bold text-white uppercase">{formData.businessName}</span>
              </div>
              <div className="flex justify-between border-b border-white/5 pb-2">
                <span className="text-gray-400 text-xs font-bold tracking-widest uppercase">Authorized Rep</span>
    ...
    <div className="w-full">
      <div className="mb-4 space-y-2">
        <div className="flex justify-between items-end">
          <span className="text-[#EAB308] text-[9px] font-black tracking-[0.2em] uppercase">Step {step} of {totalSteps}</span>
      ...
      <form onSubmit={handleSubmit} className="bg-[#111] border border-white/10 rounded-[32px] p-8 shadow-2xl min-h-[500px] flex flex-col">
        ...
        <div className="mt-8 flex gap-4">
          {step > 1 && (
            <button type="button" onClick={prevStep} className="px-6 py-4 rounded-2xl bg-white/5 border border-white/10 font-black text-xs tracking-widest text-gray-400 hover:bg-white/10 transition-all uppercase">Back</button>
          )}
          {step < totalSteps ? (
            ...
            }} className="flex-1 px-6 py-4 rounded-2xl bg-[#EAB308] text-black font-black text-xs tracking-[0.2em] hover:scale-[1.01] active:scale-95 transition-all uppercase">Continue</button>
          ) : (
            <button type="submit" className="flex-1 px-6 py-4 rounded-2xl bg-[#EAB308] text-black font-black text-xs tracking-[0.2em] hover:scale-[1.01] active:scale-95 transition-all uppercase">Initialize Desk</button>
          )}
        </div>
      </form>
    </div>
